...
void ARealGazeboManager::ConfigureSubsystem()
{
    ...
    BridgeSubsystem->UpdateFrequency = UpdateFrequency;
...
void ARealGazeboManager::ConfigurePerformanceAndDebugSettings()
{
    ...
    UE_LOG(LogRealGazeboManager, Verbose, TEXT("Performance settings configured - MaxActive: %d, UpdateRate: %.1f Hz"), 
           MaxActiveVehicles, VehicleUpdateRate);

    // Configure debug logging
    if (bLogVehicleSpawns || bLogNetworkStats || bShowPoolStatistics)
    {
        UE_LOG(LogRealGazeboManager, Log, TEXT("Debug logging enabled - Spawns: %s, Network: %s, Pool Stats: %s"), 
               bLogVehicleSpawns ? TEXT("Yes") : TEXT("No"),
               bLogNetworkStats ? TEXT("Yes") : TEXT("No"),
               bShowPoolStatistics ? TEXT("Yes") : TEXT("No"));
    }
}
...
bool ARealGazeboManager::ValidatePerformanceConfiguration() const
{
    ...
    if (VehicleUpdateRate < 10.0f || VehicleUpdateRate > 120.0f)
    {
        UE_LOG(LogRealGazeboManager, Warning, TEXT("Invalid VehicleUpdateRate: %.1f. Should be between 10-120 Hz"), VehicleUpdateRate);
        return false;
    }
    ...
    if (StatisticsUpdateInterval < 0.1f || StatisticsUpdateInterval > 10.0f)
    {
        UE_LOG(LogRealGazeboManager, Warning, TEXT("Invalid StatisticsUpdateInterval: %.1f. Should be between 0.1-10 seconds"), StatisticsUpdateInterval);
        return false;
    }
